...
ggLU <- ggplot(LU_train, aes(VISIT, MOT_MLU)) + geom_line()
ggLU
```
...
``` r
demo_train <- rename(demo_train, SUBJ = Child.ID) # giving the first column the same name for all the three data sets
...
visit1 <- select(visit1, SUBJ, ASD_level_1 = ASD_level, nonVerbalIQ_1 = nonVerbalIQ, VerbalIQ_1 = VerbalIQ, Socialization_1 = Socialization)
...
#lav levels ud af den her faktor ud fra listen ID
...
``` r
filterMLU2.7 <- filter(df_train, CHI_MLU>2.7)
filterMLU1.5 <- filter(df_train, CHI_MLU<1.5 & VISIT==1)
# vi grupperer efter subject so that it knows that it is about the subjects. Then we count how many visits each subject has gone through by counting how many times each subject appear in the dataset. Then we filter out the ones below 6 visits. 
nocomplete <- group_by(df_train, SUBJ) %>% summarize("number"=n()) %>% filter(number<6)
```
...
``` r
# tokens are words used
# desc: descending (becoming smaller)
mostwords <- filter(df_train, VISIT==6) %>% arrange(desc(tokens_CHI))

leastwords <- filter(df_train, VISIT==1) %>% arrange(tokens_CHI)
```
...
``` r
subsetting <- subset(df_train, Diagnosis = "ASD") %>% select(CHI_MLU, tokens_CHI)
```
...
``` r
# overall mean words for child
df_train$meanwords <- mean(df_train$tokens_CHI)

meanCHIwords <- group_by(df_train, SUBJ) %>% summarize("meanCHIwords"=mean(tokens_CHI))

df_train <- merge(df_train, meanCHIwords)
```
