...
library(pacman); p_load(tidyverse, lme4, ggplot2, pastecs, lmerTest)
```
...
```{r ex2, include = FALSE}
ggplot(df_train, aes(VISIT, CHI_MLU, color=Diagnosis)) +
  geom_smooth(method=lm) +
  stat_summary(fun.y = mean, geom = "point") + stat_summary(fun.y = mean, geom = "line", aes(group=Diagnosis))

# model only with main effects
mMain <- lmer(CHI_MLU~VISIT+Diagnosis+(1+VISIT|SUBJ), data=df_train)
# model with both main effects and interaction
